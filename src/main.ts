...
let Car = {
  ...
  printInfo() {
    console.log ('производитель: ' + this.manufacturer + '; модель: ' + this.model + '; год выпуска: ' + this.yearOfProduction +
    '; средняя скорость: ' + this.meanSpeed)
  },
// 2. Функция для подсчета необходимого времени для преодоления переданного расстояния со средней скоростью.
// Учтите, что через каждые 4 часа дороги водителю необходимо делать перерыв на 1 час.
  getTimeOfDistance(d:number) {
  let time = d/this.meanSpeed
  let perepiv = Math.trunc(time/4)
  if (time%4 == 0)
    perepiv--
...
Car.printInfo()
console.log(Car.getTimeOfDistance(400))
